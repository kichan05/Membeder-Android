package com.heechan.membeder.util

enum class ExtraKey(val key : String) {
    USER_DATA("user_data"),
    GOOGLE_CALL_BACK("googleCallBack"),
    TEAM_DATA("teamData"),
    CONTEST_DATA_Name("contestDataName"),
    CONTEST_DATA_Poster("contestDataPoster"),
    CONTEST_DATA_Url("contestDataUrl"),
    CONTEST_DATA_Host("contestDataHost"),
    CONTEST_DATA_Prize("contestDataPrize"),
    CONTEST_DATA_Target("contestDataTarget"),
    SCHEDULE_DATA("todoData"),
    SCHEDULE_DATA_NAME("scheduleDataName"),
    SCHEDULE_DATA_DESCRIBTION("scheduleDataDescribtion"),
    SCHEDULE_DATA_DEADLINE("scheduleDataDeadline"),
    SCHEDULE_DATA_COMPLETE("scheduleDataComplete"),
    SCHEDULE_DATA_TEAMID("scheduleDataTeamid")
}